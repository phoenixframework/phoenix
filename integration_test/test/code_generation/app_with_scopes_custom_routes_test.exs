defmodule Phoenix.Integration.CodeGeneration.AppWithScopesCustomRoutesTest do
  use Phoenix.Integration.CodeGeneratorCase, async: true

  describe "custom scope" do
    @tag database: :postgresql
    test "route_prefix and route_access_path with all generators" do
      with_installer_tmp("scopes_custom_routes", fn tmp_dir ->
        {app_root_path, _} = generate_phoenix_app(tmp_dir, "sc_routes", ["--live"])
        ...
        defmodule ScRoutes.Repo.Migrations.AddOrganizationAndOrgUser do
          use Ecto.Migration
        ...
        File.write!(
          Path.join(app_root_path, "lib/sc_routes/accounts/organization.ex"),
          """
          defmodule ScRoutes.Accounts.Organization do
            use Ecto.Schema
            import Ecto.Changeset
            alias ScRoutes.Accounts.User
        ...
        File.write!(
          Path.join(app_root_path, "lib/sc_routes/accounts/organization_user.ex"),
          """
          defmodule ScRoutes.Accounts.OrganizationUser do
            use Ecto.Schema
            import Ecto.Changeset

            schema "organizations_users" do
              field :role, :string
              belongs_to :organization, ScRoutes.Accounts.Organization
              belongs_to :user, ScRoutes.Accounts.User
        ...
        # Update context
        modify_file(Path.join(app_root_path, "lib/sc_routes/accounts.ex"), fn file ->
          inject_before_final_end(file, ~S'''
          ## Organization operations

          alias ScRoutes.Accounts.{Organization, OrganizationUser}
        ...
        modify_file(
          Path.join(app_root_path, "lib/sc_routes/accounts/scope.ex"),
          fn file ->
            String.replace(file, "defstruct user: nil", "defstruct user: nil, organization: nil")
            |> inject_before_final_end("""
            def put_organization(%__MODULE__{} = scope, %ScRoutes.Accounts.Organization{} = organization) do
        ...
        modify_file(Path.join(app_root_path, "test/support/conn_case.ex"), fn file ->
          ...
          def register_and_log_in_user_with_org(context) do
            ...
            scope = ScRoutes.AccountsFixtures.organization_scope_fixture(scope)
        ...
        modify_file(Path.join(app_root_path, "config/config.exs"), fn file ->
          String.replace(
            file,
            """
            config :sc_routes, :scopes,
              user: [
                default: true,
                module: ScRoutes.Accounts.Scope,
                assign_key: :current_scope,
                access_path: [:user, :id],
                schema_key: :user_id,
                schema_type: :id,
                schema_table: :users,
                test_data_fixture: ScRoutes.AccountsFixtures,
        ...
        modify_file(
          Path.join(app_root_path, "lib/sc_routes_web/user_auth.ex"),
          fn file ->
            inject_before_final_end(file, """
            ...
            def assign_org_to_scope(conn, _opts) do
              ...
              if slug = conn.params["slug"] do
                org = ScRoutes.Accounts.get_organization_by_slug!(current_scope, slug)
                assign(conn, :current_scope, ScRoutes.Accounts.Scope.put_organization(current_scope, org))
              else
                conn
              end
            end
                ...
                socket =
                  case socket.assigns.current_scope do
                    %{organization: nil} = scope ->
                      org = ScRoutes.Accounts.get_organization_by_slug!(socket.assigns.current_scope, slug)
        ...
        modify_file(Path.join(app_root_path, "lib/sc_routes_web/router.ex"), fn file ->
        ...
        modify_file(Path.join(app_root_path, "lib/sc_routes_web/router.ex"), fn file ->
          String.replace(
            file,
            "{ScRoutesWeb.UserAuth, :require_authenticated}",
            "{ScRoutesWeb.UserAuth, :require_authenticated}, {ScRoutesWeb.UserAuth, :assign_org_to_scope}"
          )
          |> String.replace(
        ...
        assert_file(
          Path.join(app_root_path, "lib/sc_routes_web/controllers/article_html/index.html.heex"),
          fn file ->
            assert file =~
        ...
        assert_file(
          Path.join(app_root_path, "lib/sc_routes_web/controllers/article_controller.ex"),
          fn file ->
            assert file =~
        ...
        assert_file(
          Path.join(app_root_path, "test/sc_routes_web/controllers/article_controller_test.exs"),
          fn file ->
        ...
        assert_file(
          Path.join(app_root_path, "lib/sc_routes_web/live/post_live/index.ex"),
          fn file ->
            assert file =~
        ...
        assert_file(
          Path.join(app_root_path, "lib/sc_routes_web/live/post_live/show.ex"),
          fn file ->
        ...
        assert_file(
          Path.join(app_root_path, "test/sc_routes_web/live/post_live_test.exs"),
          fn file ->
        ...
        assert_file(
          Path.join(app_root_path, "lib/sc_routes_web/controllers/comment_controller.ex"),
          fn file ->
            assert file =~
        ...
        assert_file(
          Path.join(app_root_path, "test/sc_routes_web/controllers/comment_controller_test.exs"),
          fn file ->
