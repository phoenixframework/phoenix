defmodule Phoenix.Digester do
  ...
  defp compiled_file?(file_path) do
    Regex.match?(@digested_file_regex, Path.basename(file_path)) ||
      Path.extname(file_path) == ".gz" || Path.basename(file_path) == "cache_manifest.json"
  end
  ...
  defp digest(file) do
    ...
    base16_digest = Base.encode16(:erlang.md5(file.content), case: :lower)
    digest = Base.encode64(:crypto.hash(:sha384, file.content), case: :lower)

    Map.merge(file, %{
      digested_filename: "#{name}-#{base16_digest}#{extension}",
      digest: digest
    })
  end
  ...
  defp write_to_disk(file, manifest, output_path) do
    ...
    if compress_file?(file) do
      File.write!(
        ...
        :zlib.gzip(digested_file_contents)
      )
