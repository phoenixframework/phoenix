defmodule Phoenix.Endpoint do
  ...
  The endpoint support a `:debugger` option, which you can use like this:

      defmodule YourAppWeb.Endpoint do
        use Phoenix.Endpoint,
          otp_app: :your_app,
          debugger: {MyCustomDebugger, [debugger_opt1: ..., ...]}
  ...
  This configuration option allows the user to use a custom debugger for that endpoint,
  instead of the default `Plug.Debugger`.
  It takes as arguments a pair containing a debugger module and a list of options.

  Do not mistake this option for the `:debug_errors` configuration option (which you'd set
  in your `config.exs` file).
  The `:debug_errors` controls whether the debugger should run.
  The `:debugger` option controls which debugger module will be used.
  Different endpoints in your application can use different debuggers.

  A custom debugger is a module that behaves in a similar way to the default
  `Plug.Debugger`.
  ...
  defp plug(opts) do
    quote location: :keep do
      ...
      # The :debug_errors configuration option controls whether
      # there is a debugger at all, not which debugger is run
      if var!(config)[:debug_errors] do
        unquote(debugger(opts))
      end
  ...
  defp debugger(opts) do
    case Keyword.fetch(opts, :debugger) do
      {:ok, {debugger_module, debugger_opts}} ->
        # If the user has provided a debugger, use it
        custom_debugger(debugger_module, debugger_opts)

      :error ->
        # Else, use the default debugger
        default_debugger()
    end
  end

  defp custom_debugger(module, opts) do
    all_opts = Keyword.put_new(opts, :otp_app, quote(do: @otp_app))

    quote do
      use unquote(module), unquote(all_opts)
    end
  end

  defp default_debugger() do
    quote do
      use Plug.Debugger,
        otp_app: @otp_app,
    ...
    quote location: :keep, unquote: false do
      ...
      def path(path) do
        Phoenix.Config.cache(
          __MODULE__,
          :__phoenix_path__,
          &Phoenix.Endpoint.Supervisor.path/1
        ) <> path
      end
