defmodule Mix.Tasks.Phx.Gen.Live do
  ...
  defp files_to_be_generated(%Context{schema: schema, context_app: context_app}) do
    ...
    [
      {:eex, "show.ex.eex",                Path.join([web_prefix, "live", web_path, live_subdir, "show.ex"])},
      {:eex, "index.ex.eex",               Path.join([web_prefix, "live", web_path, live_subdir, "index.ex"])},
      {:eex, "form_component.ex.eex",      Path.join([web_prefix, "live", web_path, live_subdir, "form_component.ex"])},
      ...
      {:eex, "live_test.exs.eex",          Path.join([test_prefix, "live", web_path, "#{schema.singular}_live_test.exs"])},
      {:new_eex, "modal_component.ex.eex", Path.join([web_prefix, "live", "modal_component.ex"])},
      {:new_eex, "live_helpers.ex.eex",    Path.join([web_prefix, "live", "live_helpers.ex"])},
    ]
  end
