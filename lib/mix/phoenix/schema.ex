defmodule Mix.Phoenix.Schema do
  ...
  @doc """
  Returns formatted fields and associations to list in schema.
  """
  def fields_and_associations(%Schema{} = schema) do
    schema.attrs
    |> Attribute.sort()
    |> Enum.map(&field_specifics(&1, schema))
    |> Mix.Phoenix.indent_text(spaces: 4, top: 1, bottom: 1)
  end

  defp field_specifics(%Attribute{type: :references} = attr, schema) do
    association_name = attr.options.association_name
    referenced_schema = inspect(attr.options.referenced_schema)
    foreign_key = if :"#{association_name}_id" != attr.name, do: ", foreign_key: :#{attr.name}"
    references = if attr.options[:column], do: ", references: :#{attr.options[:column]}"
    type = association_field_type(attr.options.type, schema.binary_id)

    "belongs_to :#{association_name}, #{referenced_schema}#{foreign_key}#{references}#{type}"
  end

  defp field_specifics(%Attribute{} = attr, _schema) do
  ...
  defp association_field_type(option_type, schema_binary_id) do
    if (schema_binary_id && option_type != :binary_id) ||
         (!schema_binary_id && option_type != :id),
       do: ", type: :#{option_type}"
  end
  ...
  @doc """
  Returns formatted fields to cast in schema.
  """
  ...
  @doc """
  Returns formatted fields to require in schema.
  """
  ...
  @doc """
  Returns specific changeset constraints to list in schema.
  """
  ...
  @doc """
  Returns length validations to list in schema.
  """
  ...
  @doc """
  Returns association constraints to list in schema.
  """
  ...
  @doc """
  Returns unique constraints to list in schema.
  """
