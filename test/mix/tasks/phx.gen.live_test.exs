  ...
  test "generates live resource and handles existing contexts", config do
    in_tmp_live_project(config.test, fn ->
      Gen.Live.run(~w(Blog Post posts
                      ...
                      metadata:map
                      weight:float
                      user_id:references:table,users:type,id))
      ...
      assert_file("lib/phoenix_web/live/post_live/form.ex", fn file ->
        ...
        assert file =~
                 """
                       ...
                       <.input field={@form[:slug]} label="Slug" type="text" />
                       <.input field={@form[:votes]} label="Votes" type="number" />
                       <.input field={@form[:cost]} label="Cost" type="number" step="any" />
                       <.input field={@form[:content]} label="Content" type="textarea" />
                       <.input field={@form[:tags]} label="Tags" type="select" options={["tags value", "updated tags value"]} multiple />
                       <.input field={@form[:popular]} label="Popular" type="checkbox" />
                       <.input field={@form[:drafted_at]} label="Drafted at" type="datetime-local" />
                       <.input field={@form[:status]} label="Status" type="select" options={Ecto.Enum.values(Phoenix.Blog.Post, :status)} prompt="Choose a value" />
                       <.input field={@form[:published_at]} label="Published at" type="datetime-local" />
                       <.input field={@form[:published_at_usec]} label="Published at usec" type="text" />
                       <.input field={@form[:deleted_at]} label="Deleted at" type="datetime-local" />
                       <.input field={@form[:deleted_at_usec]} label="Deleted at usec" type="text" />
                       <.input field={@form[:alarm]} label="Alarm" type="time" />
                       <.input field={@form[:alarm_usec]} label="Alarm usec" type="text" />
                       <.input field={@form[:secret]} label="Secret" type="text" />
                       <.input field={@form[:announcement_date]} label="Announcement date" type="date" />
                       <.input field={@form[:weight]} label="Weight" type="number" step="any" />
                       <.input field={@form[:user_id]} label="User" type="text" />
                 """
