...
defmodule Mix.Tasks.Phx.Gen.SchemaTest do
  ...
  test "generates references and belongs_to associations", config do
    in_tmp_project(config.test, fn ->
      Gen.Schema.run(~w(Blog.Post posts title user_id:references:*:table,users:column,id:type,id))
      assert [migration] = Path.wildcard("priv/repo/migrations/*_create_posts.exs")

      assert_file(migration, fn file ->
        assert file =~ "add :user_id, references(\"users\", on_delete: :nothing), null: false"
  ...
  test "generates references with unique indexes", config do
    in_tmp_project(config.test, fn ->
      Gen.Schema.run(
        ~w(Blog.Post posts
          title
          user_id:references:*:table,users:column,id:type,binary_id:assoc,author
          unique_category_id:references:table,categories:column,id:type,id:Catalog.Category:unique)
      )
  ...
  test "generates schema with enum", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file(path, fn file ->
        assert file =~ "create table(\"comments\")"
        assert file =~ "add :status, :string, null: false"
      end)
    end)
  end
  ...
  test "generates migration with binary_id", config do
    in_tmp_project(config.test, fn ->
      Gen.Schema.run(
        ~w(Blog.Post posts title user_id:references:*:table,users:column,id:type,binary_id --binary-id)
      )

      assert_file("lib/phoenix/blog/post.ex", fn file ->
        assert file =~ "@primary_key {:id, :binary_id, autogenerate: true}"
        assert file =~ "field :title, :string"
        assert file =~ "belongs_to :user, Phoenix.Blog.User, type: :binary_id"
      end)
  ...
  test "generates migration with custom primary key", config do
    in_tmp_project(config.test, fn ->
      Gen.Schema.run(
        ~w(Blog.Post posts title user_id:references:table,users:column,id:type,id:* --binary-id --primary-key post_id)
      )
  ...
  test "uses defaults from :generators configuration" do
    ...
    in_tmp_project("uses defaults from generators configuration (binary_id)", fn ->
      with_generator_env([binary_id: true], fn ->
        Gen.Schema.run(
          ~w(Blog.Post posts title user_id:references:*:table,users:column,id:type,id)
        )
