  ...
  test "generates with --web option", config do
    in_tmp_phx_project(config.test, fn ->
      ...
      assert_file "lib/my_app_web/templates/warehouse/user_confirmation/new.html.heex", fn file ->
        assert file =~ ~S|<.form class="phx-form" let={f} for={:user} action={Routes.warehouse_user_confirmation_path(@conn, :create)}>|
        assert file =~ ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Log in", to: Routes.warehouse_user_session_path(@conn, :new), class: "phx-link" %>|
      end
      ...
      assert_file "lib/my_app_web/templates/layout/_user_menu.html.heex", fn file ->
        assert file =~ ~S|<%= link "Settings", to: Routes.warehouse_user_settings_path(@conn, :edit), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Log out", to: Routes.warehouse_user_session_path(@conn, :delete), method: :delete, class: "phx-link" %>|
        assert file =~ ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Log in", to: Routes.warehouse_user_session_path(@conn, :new), class: "phx-link" %>|
      end
      ...
      assert_file "lib/my_app_web/templates/warehouse/user_reset_password/edit.html.heex", fn file ->
        assert file =~ ~S|<.form class="phx-form" let={f} for={@changeset} action={Routes.warehouse_user_reset_password_path(@conn, :update, @token)}>|
        assert file =~ ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Log in", to: Routes.warehouse_user_session_path(@conn, :new), class: "phx-link" %>|
      end

      assert_file "lib/my_app_web/templates/warehouse/user_reset_password/new.html.heex", fn file ->
        assert file =~ ~S|<.form class="phx-form" let={f} for={:user} action={Routes.warehouse_user_reset_password_path(@conn, :create)}>|
        assert file =~ ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Log in", to: Routes.warehouse_user_session_path(@conn, :new), class: "phx-link" %>|
      end
      ...
      assert_file "lib/my_app_web/templates/warehouse/user_session/new.html.heex", fn file ->
        assert file =~ ~S|<.form class="phx-form" let={f} for={@conn} action={Routes.warehouse_user_session_path(@conn, :create)} as={:user}>|
        assert file =~ ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new), class: "phx-link" %>|
        assert file =~ ~S|<%= link "Forgot your password?", to: Routes.warehouse_user_reset_password_path(@conn, :new), class: "phx-link" %>|
      end
      ...
      assert_file "lib/my_app_web/templates/warehouse/user_settings/edit.html.heex", fn file ->
        assert file =~ ~S|<.form class="phx-form" let={f} for={@email_changeset} action={Routes.warehouse_user_settings_path(@conn, :update)} id="update_email">|
        assert file =~ ~S|<.form class="phx-form" let={f} for={@password_changeset} action={Routes.warehouse_user_settings_path(@conn, :update)} id="update_password">|
      end
