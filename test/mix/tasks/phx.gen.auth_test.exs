  ...
  test "generates with --web option", config do
    in_tmp_phx_project(config.test, fn ->
      ...
      assert_file("lib/my_app_web/templates/warehouse/user_confirmation/new.html.eex", fn file ->
        assert file =~
                 ~S|<%= form_for :user, Routes.warehouse_user_confirmation_path(@conn, :create), fn f -> %>|

        assert file =~
                 ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new) %>|

        assert file =~
                 ~S|<%= link "Log in", to: Routes.warehouse_user_session_path(@conn, :new) %>|
      end)
      ...
      assert_file("lib/my_app_web/templates/layout/_user_menu.html.eex", fn file ->
        assert file =~
                 ~S|<%= link "Settings", to: Routes.warehouse_user_settings_path(@conn, :edit) %>|

        assert file =~
                 ~S|<%= link "Log out", to: Routes.warehouse_user_session_path(@conn, :delete), method: :delete %>|
      ...
      assert_file(
        "lib/my_app_web/templates/warehouse/user_reset_password/edit.html.eex",
        fn file ->
          assert file =~
                   ~S|<%= form_for @changeset, Routes.warehouse_user_reset_password_path(@conn, :update, @token), fn f -> %>|
      ...
      assert_file(
        "lib/my_app_web/templates/warehouse/user_reset_password/new.html.eex",
        fn file ->
          assert file =~
                   ~S|<%= form_for :user, Routes.warehouse_user_reset_password_path(@conn, :create), fn f -> %>|
      ...
      assert_file("lib/my_app_web/templates/warehouse/user_session/new.html.eex", fn file ->
        assert file =~
                 ~S|<%= form_for @conn, Routes.warehouse_user_session_path(@conn, :create), [as: :user], fn f -> %>|

        assert file =~
                 ~S|<%= link "Register", to: Routes.warehouse_user_registration_path(@conn, :new) %>|

        assert file =~
                 ~S|<%= link "Forgot your password?", to: Routes.warehouse_user_reset_password_path(@conn, :new) %>|
      end)
      ...
      assert_file("lib/my_app_web/templates/warehouse/user_settings/edit.html.eex", fn file ->
        assert file =~
                 ~S|<%= form_for @email_changeset, Routes.warehouse_user_settings_path(@conn, :update), fn f -> %>|

        assert file =~
                 ~S|<%= form_for @password_changeset, Routes.warehouse_user_settings_path(@conn, :update), fn f -> %>|
      end)
