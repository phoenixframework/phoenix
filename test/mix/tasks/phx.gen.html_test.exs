  ...
  test "generates html resource and handles existing contexts", config do
    ...
    in_tmp_project(config.test, fn ->
      ...
      assert_file("test/phoenix/blog_test.exs", fn file ->
        ...
        assert file =~
                 "deleted_at_usec: #{
                   naive_datetime |> NaiveDateTime.add(-one_day_in_seconds) |> inspect()
                 }"
        ...
        assert file =~
                 "assert post.deleted_at == #{
                   naive_datetime |> NaiveDateTime.truncate(:second) |> inspect()
                 }"
        ...
        assert file =~
                 "assert post.published_at == #{
                   datetime |> DateTime.truncate(:second) |> inspect()
                 }"
      ...
      assert_file("lib/phoenix_web/templates/post/form.html.eex", fn file ->
        ...
        assert file =~
                 ~s(<%= multiple_select f, :tags, ["Option 1": "option1", "Option 2": "option2"] %>)
        ...
        assert file =~
                 ~s|<%= select f, :status, Ecto.Enum.values(Phoenix.Blog.Post, :status), prompt: "Choose a value" %>|
