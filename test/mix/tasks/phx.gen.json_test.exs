...
defmodule Mix.Tasks.Phx.Gen.JsonTest do
  ...
  test "generates json resource", config do
    ...
    in_tmp_project(config.test, fn ->
      Gen.Json.run(~w(Blog Post posts title slug:string:unique votes:integer cost:decimal
                     ...
                     weight:float user_id:references:table,users:type,id))
