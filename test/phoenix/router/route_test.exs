defmodule Phoenix.Router.RouteTest do
  ...
  test "builds a route based on verb, path, plug, plug options and helper" do
    route = build(1, :match, :get, "/foo/:bar", nil, Hello, :world, "hello_world", [:foo, :bar], %{foo: "bar"}, %{bar: "baz"}, %{log: :debug}, HelloFoo, true)
    ...
    assert route.module == HelloFoo
    assert route.trailing_slash? == true
  end

  test "builds expressions based on the route" do
    exprs = build(1, :match, :get, "/foo/:bar", nil, Hello, :world, "hello_world", [], %{}, %{}, %{}, Hello, false) |> exprs
    ...
    exprs = build(1, :match, :get, "/", "foo.", Hello, :world, "hello_world", [:foo, :bar], %{foo: "bar"}, %{bar: "baz"}, %{}, Hello, false) |> exprs
    assert Macro.to_string(exprs.host) == "\"foo.\" <> _"

    exprs = build(1, :match, :get, "/", "foo.com", Hello, :world, "hello_world", [], %{foo: "bar"}, %{bar: "baz"}, %{}, Hello, false) |> exprs
    assert Macro.to_string(exprs.host) == "\"foo.com\""
  end

  test "builds a catch-all verb_match for match routes" do
    route = build(1, :match, :*, "/foo/:bar", nil, __MODULE__, :world, "hello_world", [:foo, :bar], %{foo: "bar"}, %{bar: "baz"}, %{}, __MODULE__, false)
    assert route.verb == :*
    assert route.kind == :match
  ...
  test "builds a catch-all verb_match for forwarded routes" do
    route = build(1, :forward, :*, "/foo/:bar", nil, __MODULE__, :world, "hello_world", [:foo, :bar], %{foo: "bar"}, %{bar: "baz"}, %{}, __MODULE__, false)
    assert route.verb == :*
  ...
  test "exprs/1 returns quoted expressions" do
    ...
    route = build(1, :match, :*, "/foo", nil, __MODULE__, opts, "hello_world", [], %{}, %{}, %{}, __MODULE__, false)
    assert {__MODULE__, escaped_opts} == exprs(route).dispatch

    fwd_route = build(1, :forward, :*, "/foo", nil, __MODULE__, opts, "hello_world", [], %{}, %{}, %{}, __MODULE__, false)
