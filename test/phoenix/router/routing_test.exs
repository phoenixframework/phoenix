defmodule Phoenix.Router.RoutingTest do
  ...
  test "get to named param with dashes" do
    ...
    conn = call(Router, :get, "users/75f6306d-a0/comments/34-95")
    assert conn.status == 200
  ...
  test "get with resources maps to new action" do
    conn = call(Router, :get, "comments/new")
    assert conn.status == 200
    assert conn.resp_body == "new comments"
  end

  test "get with resources maps to index action" do
    conn = call(Router, :get, "comments")
    assert conn.status == 200
    assert conn.resp_body == "index comments"
  end

  test "get with resources maps to show action with named param" do
    conn = call(Router, :get, "comments/123")
    assert conn.status == 200
    assert conn.resp_body == "show comments"
    assert conn.params["id"] == "123"
  end

  test "get with resources maps to edit action with named param" do
    conn = call(Router, :get, "comments/123/edit")
    assert conn.status == 200
    assert conn.resp_body == "edit comments"
    assert conn.params["id"] == "123"
  end

  test "post with resources maps to create action" do
    conn = call(Router, :post, "comments")
    assert conn.status == 200
    assert conn.resp_body == "create comments"
  end

  test "put with resources maps to update action" do
    conn = call(Router, :put, "comments/1")
    assert conn.status == 200
    assert conn.resp_body == "update comments"
    assert conn.params["id"] == "1"
  end

  test "patch with resources maps to update action" do
    conn = call(Router, :patch, "comments/2")
    assert conn.status == 200
    assert conn.resp_body == "update comments"
    assert conn.params["id"] == "2"
  end

  test "delete with resources maps to destroy action" do
    conn = call(Router, :delete, "comments/2")
    assert conn.status == 200
  ...
  test "limit resource by passing :except option" do
    conn = call(Router, :delete, "posts/2")
    assert conn.status == 404
  ...
  test "limit resource by passing :only option" do
    conn = call(Router, :put, "sessions/2")
    assert conn.status == 404
    conn = call(Router, :get, "sessions/")
    assert conn.status == 404
    conn = call(Router, :get, "sessions/1")
    assert conn.status == 404
    conn = call(Router, :get, "sessions/new")
    assert conn.status == 200
    conn = call(Router, :post, "sessions")
    assert conn.status == 200
    conn = call(Router, :delete, "sessions/1")
    assert conn.status == 200
  end
end
