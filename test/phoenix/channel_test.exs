defmodule Phoenix.Channel.ChannelTest do
  ...
  test "phoenix channel returns heartbeat message when received" do
    socket = %Socket{pid: self, router: Router, channel: "phoenix"}
  ...
  test "channel logs non-ranch formatted error" do
    socket = %Socket{pid: self, router: Router, channel: "chan7"}
    ...
    assert entry =~ "test/phoenix/channel_test.exs"
    assert entry =~ "Phoenix.Channel.ChannelTest.ErrorChannel.join/3"
  end
end
