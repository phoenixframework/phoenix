...
defmodule Mix.Tasks.Phx.New.UmbrellaTest do
  ...
  test "new with umbrella and defaults" do
    in_tmp("new with umbrella and defaults", fn ->
      ...
      assert_file(web_path(@app, "lib/#{@app}_web.ex"), fn file ->
        ...
        assert file =~ "use Phoenix.View,\n        root: \"lib/phx_umb_web/templates\""
      end)
      ...
      assert_file(
        web_path(@app, "lib/#{@app}_web/views/page_view.ex"),
      ...
      assert_file(
        web_path(@app, "lib/#{@app}_web/templates/layout/app.html.eex"),
        "<title>PhxUmb · Phoenix Framework</title>"
      )

      assert_file(
        web_path(@app, "test/#{@app}_web/views/page_view_test.exs"),
      ...
      assert_file(
        web_path(@app, "assets/js/app.js"),
        ~s[import socket from "./socket"]
      )

      assert_file(
        web_path(@app, "assets/js/socket.js"),
      ...
      assert_received {:mix_shell, :info,
                       ["Then configure your database in apps/phx_umb/config/dev.exs" <> _]}
      ...
      assert_file(
        web_path(@app, "lib/#{@app}_web/channels/user_socket.ex"),
        ~r"defmodule PhxUmbWeb.UserSocket"
      )

      assert_file(
        web_path(@app, "lib/#{@app}_web/endpoint.ex"),
        ~r"socket \"/socket\", PhxUmbWeb.UserSocket"
      )
  ...
  test "new without defaults" do
    in_tmp("new without defaults", fn ->
      ...
      assert_file(app_path(@app, "lib/#{@app}/application.ex"), "Supervisor.start_link(children,")
  ...
  test "new with binary_id" do
    in_tmp("new with binary_id", fn ->
      ...
      assert_file(
        web_path(@app, "config/config.exs"),
  ...
  test "new with path, app and module" do
    in_tmp("new with path, app and module", fn ->
      ...
      assert_file(
        "custom_path_umbrella/apps/phx_umb_web/config/config.exs",
        ~r/namespace: PhoteuxBlogWeb/
      )
      ...
      assert_file(
        "custom_path_umbrella/apps/phx_umb/config/config.exs",
        ~r/namespace: PhoteuxBlog/
      )
    end)
  end
  ...
  test "new defaults to pg adapter" do
    in_tmp("new defaults to pg adapter", fn ->
      ...
      assert_file(app_path(app, "config/dev.exs"), [
        ~r/username: "postgres"/,
        ~r/password: "postgres"/,
        ~r/hostname: "localhost"/
      ])

      assert_file(app_path(app, "config/test.exs"), [
        ~r/username: "postgres"/,
        ~r/password: "postgres"/,
        ~r/hostname: "localhost"/
      ])

      assert_file(app_path(app, "config/prod.secret.exs"), [
        ~r/username: "postgres"/,
        ~r/password: "postgres"/
      ])
      ...
      assert_file(
        web_path(app, "test/support/conn_case.ex"),
        "Ecto.Adapters.SQL.Sandbox.checkout"
      )

      assert_file(
        web_path(app, "test/support/channel_case.ex"),
        "Ecto.Adapters.SQL.Sandbox.checkout"
      )
    end)
  end
  ...
  describe "web task" do
    ...
    test "generates web-only files", %{tmp_dir: tmp_dir} do
      in_tmp(tmp_dir, fn ->
        ...
        assert_file(
          "another/lib/another/views/page_view.ex",
        ...
        assert_file(
          "another/lib/another/templates/layout/app.html.eex",
        ...
        assert_file("another/config/dev.exs", "watchers: [\n    node:")
        ...
        assert_file(
          "another/assets/js/app.js",
          ~s[import socket from "./socket"]
        )

        assert_file(
          "another/assets/js/socket.js",
          ~s[import {Socket} from "phoenix"]
        )
        ...
        assert_file(
          "another/lib/another/channels/user_socket.ex",
          ~r"defmodule Another.UserSocket"
        )
