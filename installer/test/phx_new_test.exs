...
defmodule Mix.Tasks.Phx.NewTest do
  ...
  test "new with defaults" do
    in_tmp("new with defaults", fn ->
      ...
      assert_file(
        "phx_blog/lib/phx_blog_web/templates/layout/app.html.eex",
      ...
      assert_file(
        "phx_blog/assets/js/app.js",
        ~s[import socket from "./socket"]
      )

      assert_file(
        "phx_blog/assets/js/socket.js",
      ...
      assert_file(
        "phx_blog/lib/phx_blog_web/channels/user_socket.ex",
        ~r"defmodule PhxBlogWeb.UserSocket"
      )

      assert_file(
        "phx_blog/lib/phx_blog_web/endpoint.ex",
        ~r"socket \"/socket\", PhxBlogWeb.UserSocket"
      )
  ...
  test "new defaults to pg adapter" do
    in_tmp("new defaults to pg adapter", fn ->
      ...
      assert_file("custom_path/config/prod.secret.exs", [
        ~r/username: "postgres"/,
        ~r/password: "postgres"/
      ])
  ...
  test "new without args" do
    in_tmp("new without args", fn ->
      assert capture_io(fn -> Mix.Tasks.Phx.New.run([]) end) =~ "Creates a new Phoenix project."
    end)
  end
end
