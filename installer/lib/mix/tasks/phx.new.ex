defmodule Mix.Tasks.Phx.New do
  ...
  The files generated by `mix phx.new` may be customized to meet the preferences of a developer or organization.

  Templates found in the [Phoenix repo](https://github.com/phoenixframework/phoenix/tree/main/priv/templates) may be copied into a Phoenix project and modified. Templates modified in this directory will be used when the corresponding Phoenix generator or Mix task is run.

  For example, the schema and context templates may be altered to reorganize the code and add features. Then, anytime `mix phx.gen.context` is called, the custom templates will be used to generate new modules with the customized code. Remember, test files are generated by some Mix tasks, so be sure to update them along with any changes to related modules.
  """
  use Mix.Task
