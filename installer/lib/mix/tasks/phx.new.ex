defmodule Mix.Tasks.Phx.New do
  ...
  ## Options
    ...
    * `--template-path` - use customized project templates at a specified path
  ...
  The project files generated by `phx.new` can be modified using the `--template-path` option.
  ...
  Phoenix templates are processed using EEx, so the project and module names may be injected using assigns like `@app_name` & `@app_module`.

  ### Adding Files

  In addition to modifying existing template files, the template path may include files which are not included with the default generator. For developers using [ASDF](https://asdf-vm.com/), for example, a `.tool_versions` file may be added to the `phx_single` templates to specify Elixir, Erlang, and other operating system dependencies.

  Teams using GitHub, CircleCI, Ansible, etc. may add config files where applicable inside the `phx_single` directory, and these files will be copied **unchanged** into newly generated Phoenix projects.
  ...
  official [Elixir guide](https://elixir-lang.org/getting-started/mix-otp/dependencies-and-umbrella-apps.html#umbrella-projects)
      ...
      $ mix phx.new hello_world --template-path ~/phx_templates

  Will generate a new project with templates located at `~/phx_templates`.
  """
  use Mix.Task
