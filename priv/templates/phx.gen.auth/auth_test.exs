defmodule <%= inspect auth_module %>Test do
  ...
  describe "log_in_<%= schema.singular %>/3" do
    ...
    test "redirects to settings when <%= schema.singular %> is already logged in", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      conn = conn |> assign(:current_scope, <%= schema.singular %> |> <%= inspect schema.alias %>Scope.for_<%= schema.singular %>) |> <%= inspect schema.alias %>Auth.log_in_<%= schema.singular %>(<%= schema.singular %>)
  ...
  describe "fetch_current_scope/2" do
    test "authenticates <%= schema.singular %> from session", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      <%= schema.singular %>_token = <%= inspect context.alias %>.generate_<%= schema.singular %>_session_token(<%= schema.singular %>)
      conn = conn |> put_session(:<%= schema.singular %>_token, <%= schema.singular %>_token) |> <%= inspect schema.alias %>Auth.fetch_current_scope([])
    ...
    test "authenticates <%= schema.singular %> from cookies", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      ...
      conn =
        ...
        |> <%= inspect schema.alias %>Auth.fetch_current_scope([])
    ...
    test "does not authenticate if data is missing", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      ...
      conn = <%= inspect schema.alias %>Auth.fetch_current_scope(conn, [])
      refute get_session(conn, :<%= schema.singular %>_token)
      refute conn.assigns.current_scope.<%= schema.singular %>
    end
  end
  ...
  <%= if live? do %>describe "on_mount :mount_current_scope" do
    ...
    test "assigns nil to current_scope.<%= schema.singular %> assign if there isn't a valid <%= schema.singular %>_token", %{conn: conn} do
      ...
      assert updated_socket.assigns.current_scope.<%= schema.singular %> == nil
    end

    test "assigns nil to current_scope.<%= schema.singular %> assign if there isn't a <%= schema.singular %>_token", %{conn: conn} do
  ...
  describe "on_mount :ensure_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{<%= schema.singular %>: <%= schema.singular %>} do
      ...
      socket = %LiveView.Socket{
        ...
        assigns: %{
          __changed__: %{},
          flash: %{},
          current_scope: <%= inspect schema.alias %>Scope.for_<%= schema.singular %>(%{<%= schema.singular %> | authenticated_at: eleven_minutes_ago})
        }
      }
    ...
    test "allows <%= schema.plural %> that have authenticated in the last 10 minutes", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      conn =
        conn
        |> fetch_flash()
        |> assign(:current_scope, <%= inspect schema.alias %>Scope.for_<%= schema.singular %>(<%= schema.singular %>))
    ...
    test "redirects when authentication is too old", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      ...
      conn =
        conn
        |> fetch_flash()
        |> assign(:current_scope, <%= inspect schema.alias %>Scope.for_<%= schema.singular %>(%{<%= schema.singular %> | authenticated_at: eleven_minutes_ago}))
        |> <%= inspect schema.alias %>Auth.require_sudo_mode([])
