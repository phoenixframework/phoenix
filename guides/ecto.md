...
For a thorough, general guide for Ecto, check out [Ecto's Getting Started guide](https://hexdocs.pm/ecto/getting-started.html). For an overview of all Ecto specific mix tasks for Phoenix, see the [Mix Tasks guide](phoenix_mix_tasks.html#ecto-specific-mix-tasks).
...
If we take a look at the migration generated by `mix phx.gen.schema` in `priv/repo/migrations`, we'll see that it will add the columns we specified. It will also add timestamp columns for `inserted_at` and `updated_at` which come from the `timestamps/0` function.
...
If we have an existing configuration block for our `Hello.Repo`, we can simply change the values to match our new ones. We also need to configure the correct values in the `config/test.exs` and `config/prod.secret.exs` files as well.

The last change is to open up `lib/hello/repo.ex` and make sure to set the `:adapter` to `Ecto.Adapters.MySQL`.
