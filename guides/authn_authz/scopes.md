...
Now that our scope configuration includes the `route_prefix`, and `route_access_path` is default(in our case `[:organization]`), we can generate resources scoped to the organization, and all paths will be automatically generated with the correct organization slug:
