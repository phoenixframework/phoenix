...
Because we used `mix phx.gen.auth`, we already have a real authentication system in place. We can use the `current_scope` assign to access the currently authenticated user. Let's add a new plug to `lib\hello_web\router.ex` that assigns a cart if there is an authenticated user:
