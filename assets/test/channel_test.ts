...
let channel: Channel, socket: Socket;
...
class WSMock implements WSMockInstance {
  public url: string;
...
describe("with transport", function () {
  ...
  describe("constructor", function () {
    ...
    it("sets subprotocols when authToken is provided", function () {
      ...
      socket.connect();
      expect((socket as any).conn.protocols).toEqual([
        "phoenix",
  ...
  describe("join", function () {
    ...
    it("triggers socket push with channel params", function () {
      jest.spyOn(socket, "makeRef").mockReturnValue(defaultRef.toString());
  ...
  describe("joinPush", function () {
    ...
    const helpers = {
      ...
      getBindings(event: string) {
        return channel["bindings"].filter((bind: any) => bind.event === event);
      },
    };
    ...
    beforeEach(function () {
      ...
      socket = new Socket("/socket", { timeout: defaultTimeout });
      jest.spyOn(socket, "isConnected").mockReturnValue(true);
      jest.spyOn(socket, "push").mockReturnValue(undefined);
    ...
    describe("receives 'ok'", function () {
      ...
      it("sends and empties channel's buffered pushEvents", function (done) {
        ...
        channel["pushBuffer"].push(pushEvent as any);
  ...
  describe("onError", function () {
    ...
    it("does not rejoin if channel leaving", function () {
      (channel as any).state = "leaving";

      const spy = jest.spyOn(joinPush, "send");

      (socket as any).onConnError({});
  ...
  describe("onMessage", function () {
    it("returns payload by default", function () {
      ...
      channel = socket.channel("topic", { one: "two" });
      jest.spyOn(socket, "makeRef").mockReturnValue(defaultRef.toString());
      const payload = channel.onMessage(
        "event",
        { one: "two" },
        defaultRef.toString(),
      );
  ...
  describe("on", function () {
    ...
    it("sets up callback for event", function () {
      ...
      channel.trigger("event", {}, defaultRef.toString());
      expect(spy).not.toHaveBeenCalled();
  ...
  describe("off", function () {
    ...
    it("removes all callbacks for event", function () {
      ...
      channel.trigger("event", {}, defaultRef.toString());
      channel.trigger("other", {}, defaultRef.toString());
  ...
  describe("push", function () {
    ...
    const pushParams = (channel: Channel) => {
      return {
        topic: "topic",
        event: "event",
        payload: { foo: "bar" },
