...
class Push {
  ...
  matchReceive({ status, response, ref }) {
    this.recHooks.forEach(h =>
      h.status === status && h.callback(response)
    )
  }
...
export class Channel {
  ...
  trigger(event, payload, ref) {
    ...
    const handledPayload = this.onMessage(event, payload, ref)
    ...
    this.bindings.forEach(bind =>
      bind.event === event && bind.callback(handledPayload, ref)
    )
  }
  replyEventName(ref) {
...
export class Socket {
  ...
  constructor(endPoint, opts = {}) {
    ...
    this.reconnectAfterMs = opts.reconnectAfterMs || ((tries) => 
  ...
  connect(params) {
    if (params) {
      console && console.warn("passing params to connect is deprecated. Instead pass :params to the Socket constructor")
      this.params = params
    }
  ...
  onConnMessage(rawMessage) {
    const handler = (msg) => {
      ...
      const logMsg = `${payload.status || ""} ${topic} ${event}${ref ? ` (${ref})` : ""}`
      this.log("receive", logMsg, payload)

      this.channels.forEach(channel =>
        channel.isMember(topic) && channel.trigger(event, payload, ref)
      )

      this.stateChangeCallbacks.message.forEach(callback => callback(msg))
  }

    this.decode(rawMessage.data, handler)
  }
}
...
export const Presence = {
  ...
  syncDiff(currentState = {}, { joins, leaves }, onJoin = noop, onLeave = noop) {
    const state = this.clone(currentState)
  ...
  list(presences, chooser) {
    ...
    return this.map(presences, chooser)
  },

  // private
  map(obj, func) {
    return Object.getOwnPropertyNames(obj)
      .map(key => func(key, obj[key]))
  },
